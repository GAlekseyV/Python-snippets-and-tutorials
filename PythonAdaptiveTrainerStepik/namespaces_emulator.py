...
# латинских букв. Формат выходных данных
...
while n > 0:
    ...
    if cmd == 'add':
        stack[namespace].append(var)
    elif cmd == 'create':
        stack[namespace] = [var]
    elif cmd == 'get':
        cur_namespace = namespace
        while cur_namespace != "global":
            if var in stack[cur_namespace]:
                break
            cur_namespace = stack[cur_namespace][0]
        else:
            if var not in stack["global"]:
                cur_namespace = None
        print(cur_namespace)
    n -= 1
